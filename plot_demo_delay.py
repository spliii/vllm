...
#  python plot_eval_t_token.py --engine 0 --dataset 1 --qps 2 --fontsize 16 --k 20 --token 1000 --ttft 1.0 --rid 403 --x 20 --y 400 --tbt 0
...
parser.add_argument("--tbt", type=float, default=0)
...
if matching_files:
    ...
    try:
        with open(file_path, 'r') as file:
            ...
            for i in range (0, 1):
                if token > len(data['itls'][request_id[0]]):
                    token = len(data['itls'][request_id[0]])
                sublist = data['itls'][request_id[0]][:token]
                # print(sublist)
                sublist[0] = args.ttft
...
smoothed_list = [[]]
smoothed_list[0] = itls_list[0].copy()

# 平滑delay
tbt = args.tbt
if tbt > 0:
    print("Smoothing delay: tbt={}".format(tbt))
    for i in range(1, len(smoothed_list[0])):
        if smoothed_list[0][i] - smoothed_list[0][i-1] <= tbt:
            smoothed_list[0][i] = smoothed_list[0][i-1] + tbt
...
ax.plot(itls_list[0], list(range(1, len(itls_list[0]) + 1)), color='tab:blue', linewidth=lw, label='w/o Output Delay')
ax.scatter(itls_list[0], list(range(1, len(itls_list[0]) + 1)), color='tab:blue', s=4)

ax.plot(smoothed_list[0], list(range(1, len(smoothed_list[0]) + 1)), color='tab:green', linewidth=lw, label='w/ Output Delay')
ax.scatter(smoothed_list[0], list(range(1, len(smoothed_list[0]) + 1)), color='tab:green', s=2)
...
leg = plt.legend(fontsize=fontsize-2, frameon=False, loc='upper left', handlelength=1)
